...
class Agent:
    ...
    def best_state(self, states):
        ...
        if random.random() <= self.epsilon:
            a=random.choice(list(states))
            # print("Best state :",a[1])
            return a[1]
    # 2 2 2 2 
    # 2 4 2 8
    # 
    # 
            ...
            for state in states:
                # print("Best states: ",state)
                value = self.predict_value(np.reshape(state[1], [1, self.state_size]))
    ...
    def train(self, batch_size=32, epochs=3):
        ...
        if n >= self.replay_start_size and n >= batch_size:
            ...
            next_qs = [x[0] for x in self.model.predict(
                next_states
                )]
    ...
    def save(self, name):
        self.model.save("./models/2048_no_"+str(name)+".model")

    def load(self,name):
        self.model=load_model("./models/2048_no_"+str(name)+".model")